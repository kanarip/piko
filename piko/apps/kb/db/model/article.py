...
class KBArticle(Translatable, db.Model):
    ...
    def votes_down(self):
        ...
        """
        return db.session.query(KBVote).filter_by(article_id=self._id, positive=False).count()
    ...
    def votes_up(self):
        ...
        """
        return db.session.query(KBVote).filter_by(article_id=self._id, positive=True).count()
